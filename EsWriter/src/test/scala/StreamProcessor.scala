    ...
    stream.foreachRDD(rdd => {
      ...
      if(!rdd.isEmpty()){
        ...
        val unpackedDataframe = Unpacker.unpack(df, nHits)
          ...
          //.cache()
        //println(s"Hits: ${unpackedDataframe.count()}")
        ...
        val events = eventBuilder
          .select("RUN_ID", "ORBIT_CNT", "X_POS_LEFT", "X_POS_RIGHT", "Z_POS", "TDRIFT")
        ...
        groupedEvents
          ...
          .select("RUN_ID", "TIME_STAMP", "ORBIT_CNT", "NHITS", "HITS_LIST").saveToEs("run-{RUN_ID}")
